...
function drawEmbedding (data, onSelectedLabel) {
  // Retrieve all data
  ...
  g.append("svg:circle")
    ...
    .attr("opacity", 1)
    .attr("r", 6)
    .on("mouseover", d => { if (onSelectedLabel) onSelectedLabel(d.index) })
    .on("mouseout", d => { if (onSelectedLabel) onSelectedLabel(-1) })
...
class Renderer2D extends React.Component {
  ...
  componentWillReceiveProps (nextProps) {
    if (nextProps.meta_data != this.props.meta_data) {
      drawEmbedding(nextProps.meta_data, this.props.onSelectedLabel)
    }
  ...
  componentDidMount () {
    ...
    if (this.props.meta_data != undefined) {
      drawEmbedding(this.props.meta_data, this.props.onSelectedLabel)
    }
  }
